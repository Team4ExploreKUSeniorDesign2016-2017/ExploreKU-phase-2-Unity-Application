...
public class MoveWithCompass : MonoBehaviour
{
	...
	void Update()
	{
		...
		Quaternion gyroOrientation = Quaternion.Euler (90, 0, 0) * Input.gyro.attitude * Quaternion.Euler(0, 0, -180);
		...
		if (Input.compass.timestamp > _lastCompassUpdateTime)
		{
			...
			_compassOrientation = Quaternion.Euler (180, 0, 0) * Quaternion.Inverse(Quaternion.LookRotation(flatNorth, -gravity)) * Quaternion.Euler (0, 0, -180);
